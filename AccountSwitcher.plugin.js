...
module.exports = (() => {
	const config = {
		info: {
			...
			version: "1.3.2",
			description:
				"Simply switch between accounts with the ease of pressing a single key.",
			github: "https://github.com/l0c4lh057/AccountSwitcher",
			github_raw:
				"https://raw.githubusercontent.com/l0c4lh057/AccountSwitcher/master/AccountSwitcher.plugin.js",
		},
		changelog: [
			{
				title: "Changed",
			...
			},
			{
				...
				type: "fixed",
				items: [
					"Showing the keybinds in the settings again. (The toggle for encryption still does not work but that is added by ZeresPluginLibrary which I do not control.)",
				],
			},
		],
	};
	...
	return !global.ZeresPluginLibrary
		...
		: (([Plugin, Api]) => {
				const plugin = (Plugin, Api) => {
					...
					const unregisterKeybind = WebpackModules.getByProps(
						"inputEventUnregister"
					).inputEventUnregister.bind(
						WebpackModules.getByProps("inputEventUnregister")
					);
					const registerKeybind = WebpackModules.getByProps(
						"inputEventRegister"
					).inputEventRegister.bind(
						WebpackModules.getByProps("inputEventUnregister")
					);
					...
					const KeyRecorder = class KeyRecorder extends WebpackModules.getByDisplayName(
						"KeyRecorder"
					) {
						render() {
							...
							ret.props.children.props.children.props.children.push(
								React.createElement(
									...
									React.createElement(
										Button,
										{
											className: WebpackModules.getByProps(
												"editIcon",
												"button"
											).button.split(" ")[1],
					...
					`
							...
							if (this.settings.encryptionVersion !== 2) {
								if (!this.settings.encrypted) {
									this.settings.accounts.forEach((acc) => {
										acc.token = this.encryptUpdated(
											this.decryptDeprecated(acc.token, acc.id),
											acc.id
										);
									});
									...
									this.requirePassword(
										"The encryption algorithm got updated for better security. Please enter your password to decrypt your tokens and encrypt them again with the new algorithm."
									).then(() => {
										this.settings.encTest = this.encryptUpdated(
											"test",
											password
										);
										this.settings.accounts.forEach((acc) => {
											acc.token = this.encryptUpdated(
												this.encryptUpdated(
													this.decryptDeprecated(
														this.decryptDeprecated(acc.token, password),
														acc.id
													),
													acc.id
												),
									...
									React.createElement(
										"div",
										{
											...
											onClick: (e) => {
												...
												this.saveSettings();
												Toasts.show("Account " + account.name + " removed", {
							...
							if (this.settings.accounts.length > 0) {
								...
								ReactDOM.render(
									React.createElement(
										...
										this.settings.accounts.map((account) =>
											React.createElement(AccountPanel, account)
										)
									),
									menu
								);
								...
								Toasts.show("No accounts to display", {
									type: Toasts.ToastTypes.warning,
								});
							}
							const eventHandler = (ev) => {
								if (!ev.target || !ev.target.classList) return;
								if (
									!ev.target.classList.contains("accountswitcher-switchmenu") &&
									!ev.target.classList.contains("accountswitcher-removeaccount")
								) {
									menu.remove();
						...
						login(account) {
							...
							if (account.id == UserStore.getCurrentUser().id)
								return Toasts.show("Already using account " + account.name, {
									type: Toasts.ToastTypes.warning,
								});
							console.log("Logging in as " + account.name);
						...
						getSettingsPanel() {
							...
							const accountsField = new Settings.SettingGroup("Accounts", {
								shown: true,
							});
							const addAccount = (account) => {
								if (!account) {
									...
									let acc = {
										...
										token: this.settings.encrypted
											? this.encrypt(t, password)
											: t,
									};
								...
								const kbPanel = new Keybind(
									...
									() => {
										...
										Toasts.show(
											"Account " +
												account.name +
												" got removed. After reopening the settings it will also be gone from this list.",
											{ type: Toasts.ToastTypes.success }
										);
									}
								);
							...
							addAccountButton.className =
								"button-38aScr lookFilled-1Gx00P colorBrand-3pXr91 sizeMedium-1AC_Sl grow-q77ONN";
							addAccountButton.addEventListener("click", () => {
								if (
									this.settings.accounts.some(
										(acc) => acc.id == UserStore.getCurrentUser().id
									)
								) {
									return Toasts.show("You already saved this account", {
							...
							new Settings.SettingGroup(this.getName(), { shown: true })
								.appendTo(panel)
								.append(
									new Settings.Switch(
										"Encrypt tokens",
										"Encrypting tokens makes sure that nobody will be able to get the tokens without knowing the password.",
										this.settings.encrypted,
										(checked) => {
												...
												const retry = () => {
													...
													Modals.showModal(
														...
														),
														{
															onConfirm: () => {
																if (pw1 != pw2) {
																	Toasts.show("Passwords don't match", {
																		type: Toasts.ToastTypes.error,
																	});
																	return retry();
																}
																...
																this.settings.encTest = this.encrypt(
																	"test",
																	password
																);
																this.settings.accounts.forEach(
																	(acc) =>
																		(acc.token = this.encrypt(
																			acc.token,
																			password
																		))
																);
													...
													Modals.showModal(
														...
														React.createElement(
															...
															React.createElement(
																Markdown,
																{},
																"Are you sure that you want to disable encryption? To verify please input your current password. You can also choose the 'Forgot Password' option which will remove all saved accounts. To abort just click outside of this popout."
															),
															React.createElement("input", {
																type: "password",
																placeholder: "Password",
														...
														),
														{
															onConfirm: () => {
																try {
																	if (
																		this.decrypt(this.settings.encTest, pw) !==
																		"test"
																	) {
																		Toasts.show("Passwords incorrect", {
																			type: Toasts.ToastTypes.error,
																		});
																		return retry();
																	}
																	...
																	this.settings.accounts.forEach(
																		(acc) =>
																			(acc.token = this.decrypt(acc.token, pw))
																	);
																	password = null;
																	this.saveSettings();
																} catch (ex) {
															...
															onCancel: () => {
																Modals.showConfirmationModal(
																	"Are you sure?",
																	"You are about to disable encryption which will remove all your currently saved accounts without an option to recover them. Only use this if you really forgot your password.",
																	{
																		onConfirm: () => {
																			this.settings.encTest = "test";
																			this.settings.encrypted = false;
								...
								.append(
									new Settings.Textbox(
										"Plugins to restart",
										"Put the name of all plugins that should get restarted when you switch accounts in this textbox separated by a comma",
										this.settings.pluginsToRestart.join(","),
										(val) => {
											this.settings.pluginsToRestart = val
												.split(",")
												.map((x) => x.trim())
												.filter((x) => x);
											this.saveSettings();
						...
						async requirePassword(message) {
							if (!this.settings.encrypted || password !== null)
									...
									Modals.showModal(
										...
										),
										{
											onConfirm: () => {
												try {
													if (
														this.decrypt(this.settings.encTest, pw) !== "test"
													) {
														Toasts.show("Wrong password", {
															type: Toasts.ToastTypes.error,
														});
														return retry(t + 1);
													}
						...
						encrypt(text, pw) {
							return this.settings.encryptionVersion === 2
								? this.encryptUpdated(text, pw)
								: this.encryptDeprecated(text, pw);
						}
						decrypt(text, pw) {
							return this.settings.encryptionVersion === 2
								? this.decryptUpdated(text, pw)
								: this.decryptDeprecated(text, pw);
						}
						...
						loadSettings() {
							...
							if (!Array.isArray(this.settings.accounts))
								this.settings.accounts = Object.values(this.settings.accounts);
							this.settings.accounts.forEach((acc) => {
								if (!Array.isArray(acc.keybind))
									acc.keybind = Object.values(acc.keybind);
							});
						}
						saveSettings() {
